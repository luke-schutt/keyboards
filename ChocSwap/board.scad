...
module half_hand_negative(flip = false) {
    translate([0, spacing[1] * 1.5, 0])
        rotate([0, 0, half_angle * (flip ? 1 : -1)]) {
            scale_if(flip, [-1, 1, 1]) {
                translate([-spacing[0] / 2, 0, 3]) {
                    columns(half_spec, spacing = spacing)
                        scale_if(flip, [-1, 1, 1]) key_fill();
                }
                translate([spacing[1] * 1.5 - 1.25, spacing[0] * -3.75 + 3, 3])
                    rotate([0, 0, 270])
                    column([1, 1, 1], spacing = spacing[1])
                    scale_if(flip, [-1, 1, 1]) key_fill(size = 1.5);
            }
            if (!switch_dummy && route_wires) {
                wire_x = switch_spacing * -0.5 + 0.25;
                scale_if(flip, [-1, 1, 1]) {
                    translate([wire_x, -spacing[1] + 15, 3.01])
                        row_wire_negative();
                    translate([wire_x, -spacing[1] * 2 + 15, 3.01])
                        row_wire_negative();
                    translate([wire_x, -spacing[1] * 3 + 15, 3.01])
                        row_wire_negative();
                    translate([spacing[0] * 1.5 - 4.5, spacing[1] * -3.5 - 11.5, 3.01])
                        rotate([0, 180, 90])
                        wire_channel(spacing[1] * 3 - 3, rounded = round_wires);
                }
            }
        }
}
